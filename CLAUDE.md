...
**Map overlay (semantic navigation).** The `map` toolbar button calls `/api/summarize` with the full graph snapshot; the response streams into a modal overlay rendered with Source Serif 4. The summary references cards inline via `[card:shape:xxx]` syntax; the renderer (`MapOverlay` in `App.tsx`) splits the streaming text into prose + ref segments, rendering each ref as a small "jump" pill. Clicking a pill closes the overlay and `editor.centerOnPoint` pans to the card. Stale ids (cards deleted since the snapshot) render disabled. Built on the existing `get_graph_summary` foundation but uses a one-shot Messages call (faster + cheaper than spinning up a Managed Agent session for pure synthesis).
