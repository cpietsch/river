...
**Map nav-bar (semantic navigation).** The `map` toolbar button calls `/api/summarize`; the response streams into a thin floating strip below the toolbar (`MapBar` in `App.tsx`) — translucent backdrop, single horizontal line, max ~720px, jump-pills inline. The canvas remains interactive while the bar is open. The summary references cards via `[card:shape:xxx]` syntax; the renderer parses these into small `↗` pills that pan the camera to the card. Stale ids render disabled. ESC or × closes. Routes through the same brain agent as `/api/generate` so it can use the existing `get_graph_summary` / `get_card` custom tools — the kickoff message mode-shifts the brain into nav mode (overrides default LENGTH/FORMATTING). The graph snapshot is embedded inline in the kickoff so the agent has everything in one shot; it can still call `get_card` to drill in if it needs more detail. Memory store is skipped (no persistent context needed for navigation).
